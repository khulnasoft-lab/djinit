...
// Watch
function watchPaths() {
  watch(`${paths.sass}/*.scss`{% if djinit.windows == 'y' %}, { usePolling: true }{% endif %}, styles);
  watch(`${paths.templates}/**/*.html`{% if djinit.windows == 'y' %}, { usePolling: true }{% endif %}).on('change', reload);
  watch([`${paths.js}/*.js`, `!${paths.js}/*.min.js`]{% if djinit.windows == 'y' %}, { usePolling: true }{% endif %}, scripts).on(
    'change',
    reload,
  );
}
